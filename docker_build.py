...
def build_env_images(
    ...
    with open('/home/snoronha/miniconda/envs/agentless/lib/python3.11/site-packages/swebench/harness/env_image.txt', 'r') as file:
        content = file.read()

    for image_name, config in configs_to_build.items():
        
        if image_name == "sweb.env.x86_64.7037e8c448a4b8ebfe9b13:latest":
            config["setup_script"] = content 
        print(config["setup_script"])
